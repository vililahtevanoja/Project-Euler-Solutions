...
class SuffixTree {
  case class Node(edgeTo: Option[Edge], edgesFrom: Option[Array[Edge]]) 
  case class Edge(startNode: Node, s: String, endNode: Node)
    
  def create(str: String): Node = {
      
  }

  def append(root: Node, chr: Char): Node {
